...
export const ttfToWoff = () => {
    ...
    return app.gulp.src(`${app.path.srcFolder}/fonts/*.ttf`, { encoding: false })
        ...
        // Конвертация в .woff2
        .pipe(ttf2woff2())
        // Выгружаем в папку с резльтатом
        .pipe(app.gulp.dest(`${app.path.build.fonts}`))
}
export const fontsStyle = () => {
    //Файл стилей подключения шрифтов
    let fontsFile = `${app.path.srcFolder}/scss/modules/_fonts.scss`;
    // Проверка сущестуют ли файлы шрифтов
    fs.readdir(app.path.build.fonts, function (err, fontsFiles) {
        if (fontsFiles) {
            ...
            if (!fs.existsSync(fontsFile)) {
                ...
                console.log('Файл scss/modules/_fonts.scss уже существует. Для обновления файл нужно удалить!');
            }
        }
    });
